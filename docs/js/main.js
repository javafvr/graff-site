$(document).ready(function() {
	...
	$("nav a,a[href='#top'],a[rel='m_PageScroll2id'],a.PageScroll2id, a.scroll-down").mPageScroll2id({
		highlightSelector: 'nav a  , a.scroll-down',
		offset: 30
	});
	...
	// Отменяет JS tilt на экранах меньше 768
	function toggleTilt(){
		const tilt = $('.js-tilt').tilt();
		var w = $(window).width();
       	var element = $('.js-tilt');
		if(w < 768){
		    const element = $('.js-parent').closest('.js-tilt');
		    console.log(element);
	        element.tilt.destroy.call(element);
		} else{
	        var element = $('.logo').closest('.js-parent').find('.js-tilt');
